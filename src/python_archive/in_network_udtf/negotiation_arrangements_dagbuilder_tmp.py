...
def iterate_define_ddl(p_session: Session ,p_root_task_name: str ,p_approx_batch_size: int 
    ,p_stage_path: str ,p_datafile: str ,p_target_stage: str 
    ,p_buckets: int ,p_execution_wh: str): 
    ...
    max_rec_num = 100*500
    l_buckets = p_buckets #max(BUCKETS ,p_buckets) - 4;
    ranges = split_range_into_buckets(max_rec_num ,l_buckets)
    tm = get_basename_of_datafile(p_datafile)
    ...
    for idx,(m ,n) in enumerate(ranges):
        ...
        ddl = f'''
            create or replace task {task_name}
                WAREHOUSE = {p_execution_wh}
                comment = 'negotiated_arrangements segment range [{m} - {n}] data ingestor '
                after {p_root_task_name}
                as
...
def main(p_session: Session ,p_approx_batch_size: int 
    ...
    root_task_name = f'''DAG_INNETWORK_LOAD_{get_basename_of_datafile(p_datafile)}'''
    create_root_task_and_fh_loader(p_session ,root_task_name ,p_stage_path ,p_datafile)

    task_def_errors = []
    task_ddls = iterate_define_ddl(p_session ,root_task_name ,p_approx_batch_size 
        ,p_stage_path ,p_datafile ,p_target_stage
        ,p_buckets ,p_execution_wh)
    idx = 0
