...
def parse_breakdown_save(p_session: Session ,p_approx_batch_size: int ,p_stage_path: str ,p_datafile: str ,p_target_stage: str ,f):
    ...
    files_written = -1
    seq_no = -1
    for idx ,rec in enumerate(ijson.items(f, 'in_network.item')):
        seq_no += 1

        if seq_no < p_from_seq:
            continue
        elif seq_no > p_to_seq:
            break
        ...
        for chunk_idx ,chunk in enumerate( segment_child_chunks ):
            ...
            out_file = os.path.join(out_folder ,f'{segment_id}' , f'data_{seq_no}_{chunk_idx}.json.gz')
...
def parse_breakdown_save_wrapper(p_session: Session ,p_approx_batch_size: int ,p_stage_path: str ,p_datafile: str ,p_target_stage: str):
    logger.info('Parsing and breaking down in_network ...')
    rec_count = -1
    ...
    if json_fl.endswith('.json'):
        with _snowflake.open(json_fl) as f:
            rec_count = parse_breakdown_save(p_session ,p_approx_batch_size ,p_stage_path ,p_datafile ,p_target_stage ,f)
        ...
        with ZipFile(_snowflake.open(json_fl)) as zf:
            for file in zf.namelist():
                with zf.open(file) as f:
                    rec_count ,rdata = parse_breakdown_save(p_session ,p_approx_batch_size ,p_stage_path ,p_datafile ,p_target_stage ,f)
    
    # return rec_count
    return rdata

def main(p_session: Session ,p_approx_batch_size: int ,p_stage_path: str  ,p_datafile: str ,p_target_stage: str):
    ret = {}
    ret['data_file'] = p_datafile
    
    seg_record_counts = parse_breakdown_save_wrapper(p_session ,p_approx_batch_size ,p_stage_path ,p_datafile ,p_target_stage)
